...
def FCFS(jarjend):
    ...
    kogu_ooteaeg = 0
    # paneb protsesse vajalikku järjekorda
    for p in sorted(jarjend, reverse=False):
        saabumine = p[0]
        kestus = p[1]
        if saabumine > (jarg):
                # kui kahe protsessi vahel on "auk", siis jäetakse sinna delay näitamiseks õige pikkusega tühik
...
def SJF(jarjend):
    ...
    for p in jarjend:
        ...
        if saabumine > (jarg):
            ...
            counter += 1
        try:
            if jarjend[a+1][1] > jarjend[a+2][1] and jarjend[a+1][0] <= jarg:
                jarjend[a+1], jarjend[a+2] = jarjend[a+2], jarjend[a+1]
            a += 1
        except IndexError:
            pass
...
FCFS_nupp.place(x=190, y=190,height=25, width=80)

NJF_nupp = ttk.Button(raam, text="NJF", command = lambda : jooksuta_algoritmi("NJF"))
NJF_nupp.place(x=280, y=190,height=25, width=80)

RR_nupp = ttk.Button(raam, text="RR", command = lambda : jooksuta_algoritmi("RR"))
RR_nupp.place(x=370, y=190,height=25, width=80)
